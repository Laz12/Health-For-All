<form method="post" action="" name="signup-form">
    ...
    <div class="form-element">
        <label>Update Medical_Check_up_date</label>
        <input type="text" name="Medical_Check_up_date" required />
    </div>
    <div class="form-element">
        <label>Update Medical_Check_up_time</label>
        <input type="text" name="Medical_Check_up_time" required />
    </div>
    <div class="form-element">
        <label>Update Medical_Check_up_reason</label>
        <input type="text" name="Medical_Check_up_reason" required />
    </div>
    <div class="form-element">
        <label>Update Medical_Check_up_documentation</label>
...
if (isset($_POST['register'])) {
    ...
    $query->bindParam("Medical_Check_up_ID", $Medical_Check_up_ID, PDO::PARAM_INT);
    $query->execute();
    if ($query->rowCount() > 0) {
        $query = $connection->prepare("UPDATE Medical_Check_up SET Medical_Check_up_date=:Medical_Check_up_date, Medical_Check_up_time=:Medical_Check_up_time, Medical_Check_up_reason=:Medical_Check_up_reason, Medical_Check_up_documentation=:Medical_Check_up_documentation WHERE Medical_Check_up_ID=:Medical_Check_up_ID");
        $query->bindParam("Medical_Check_up_ID", $Medical_Check_up_ID, PDO::PARAM_INT);
    ...
    header("Location: ../Doctor_credential/dmedical_check_up.php");
}
?>